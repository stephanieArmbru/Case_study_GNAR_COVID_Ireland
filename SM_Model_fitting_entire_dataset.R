...
mase_arima <- fit_and_predict_arima(forecast_window = 5)
...
knn_best_res <- knn_best_df[which.min(knn_best_df$best.model.old.BIC), c(2, 3)]
colnames(knn_best_res) <- c("model", "BIC")
...
dnn_best_res <- dnn_best_df[which.min(dnn_best_df$best.model.old.BIC), c(2, 3)]
colnames(dnn_best_res) <- c("model", "BIC")
...
# SPL weighting 
# for latex 
bic_development <- cbind(results_old_delaunay[, c(1, 3)], 
                         results_old_gabriel[, 3], 
                         results_old_soi[, 3], 
                         results_old_relative[, 3], 
                         results_old_queen[, 3], 
                         # results_old_eco_hubs[, 3], 
                         results_old_train[, 3])

bic_development_df <- left_join(bic_development, 
        results_old_eco_hubs[, c(1, 3)], 
        by = "name")


strCaption <- "\\code{GNAR} models with SPL weighting \\textbf{Delaunay triangulation}, \\textbf{Gabriel}, \\textbf{SOI}, 
\\textbf{Relative neighbourhood}, \\textbf{Queen's contiguity}, \\textbf{Railway-based} and \\textbf{Economic hub} network
and the corresponding
BIC for model selection; the best performing model for each network is 
highlighted in bold red"
print(xtable(bic_development_df,
             digits=2,
             caption=strCaption,
             label="tab:gnar_all", 
...
# for latex
strCaption <- "Comparison of best performing GNAR models for
...
print(xtable(compare_df,
      ...
      add.to.row = list(pos = list(-1,
                                   nrow(compare_df)),
                        command = c(paste("\\toprule \n",
                                          " Network & hyperparameter & GNAR model & BIC \\\\\n",
                                          "\\midrule \n"),
                                    "\\bottomrule \n")
      )
)
...
knn_best_res$network <- "KNN (k = 21)"
dnn_best_res$network <- "DNN (d = 325)"

knn_best_res$hyperparameters <- "SPL"
dnn_best_res$hyperparameters <- "SPL"
...
# best for each network 
compare_df_best <- compare_df %>% 
  group_by(network) %>% 
  filter(BIC == min(BIC) %>% as.numeric()) %>% 
  as.data.frame()

compare_df_best %>% arrange(BIC)
...
# compare_df <- rbind("Train" = return_best_model(results_old_train),
#                     "Queen" = return_best_model(results_old_queen), 
#                     "Eco hubs" = return_best_model(results_old_eco_hubs), 
#                     "KNN (k = 21)" = knn_best_res, 
#                     "DNN (d = 325)" = dnn_best_res, 
#                     "Delaunay" = return_best_model(results_old_delaunay), 
#                     "Gabriel" = return_best_model(results_old_gabriel), 
#                     "Relative" = return_best_model(results_old_relative), 
#                     "SOI" = return_best_model(results_old_soi), 
#                     "Complete" = return_best_model(results_old_complete)
# )
# 
# compare_df$network <- rownames(compare_df)
# 
# compare_df %>% arrange(BIC)

# for latex 
strCaption <- "Overview over best performing GNAR models for 
each COVID-19 network, with best performing weighting scheme;
best performing GNAR model in bold face."
print(xtable(compare_df_best,
             digits=2,
             caption=strCaption,
      ...
      hline.after=NULL,
      add.to.row = list(pos = list(-1,
                                   nrow(compare_df_best)),
                        command = c(paste("\\toprule \n",
                                          "Network & Weighting scheme & GNAR model &
                                          BIC \\\\\n",
                                          "\\midrule \n"),
                                    "\\bottomrule \n")
      )
...
# Eco hubs
model_eco_hubs <- fit_and_predict(alpha = 7, 
                                  beta = c(3, 1, 1, 1, 0, 0, 0),
                                  globalalpha = TRUE, 
...
# SOI
model_soi <- fit_and_predict(alpha = 7, 
                             beta = c(4, 2, 2, 0, 0, 0, 0),
                             globalalpha = TRUE, 
...
# MASE --------------------------------------------------------------------
# compute MASE for the last 5 weeks 
